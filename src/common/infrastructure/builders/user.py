...
from src.common.domain.entities.user import (
    User,
)
...
def build_user(orm_instance: UserORM) -> User:
    return User(
        ...
        email_address=(
            build_email_address(orm_instance.email_address)
            if orm_instance.email_address else None
        ),
        phone_number=(
            build_phone_number(orm_instance.phone_number)
            if orm_instance.phone_number else None
        ),
